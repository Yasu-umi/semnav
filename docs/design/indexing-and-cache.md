...
* **Target**: Walk the tree under `workspaceFolders` (rootUri) and collect files with the target language extensions (`.py` / `.ts` / `.tsx` / `.rs` / `.go`, plus any `SEMNAV_LSP_<TAG>_EXTENSIONS` registered via `SEMNAV_CUSTOM_LANGUAGES` — [language-adapters.md](./language-adapters.md) "Custom/Generic Adapter")
