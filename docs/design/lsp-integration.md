...
These findings were originally captured with an ad hoc harness driving pyright and typescript-language-server directly over stdio against scratch fixtures that were never checked into the repo. Most of them are now pinned as regression tests against real captured server responses under [`tests/fixtures/lsp-probe/`](../../tests/fixtures/lsp-probe/):

* Real-server, end-to-end (need node/npm; `#[ignore]`d by default, run locally with `cargo test -- --ignored`): `callhierarchy_incoming_from_module_scope_synthesizes_module_caller`, `callees_handles_pyright_null_outgoing_calls_as_empty`, `references_resolve_cross_file_without_preemptive_didopen`, `workspace_symbol_returns_empty_array_for_pyright` (`src/query/pool.rs`); `index_language_real_pyright`, `index_language_real_tsserver` (`src/indexer/runner.rs`).
* Fast, offline, against a captured response (ordinary `cargo test`, no LSP server needed): `parse_locations_matches_captured_pyright_definition`, `parse_locations_matches_captured_tsserver_locationlink` (`src/query/lsp_query.rs`); `construct_from_hover_ignores_self_at_classname_notation`, `construct_from_hover_extracts_type_keyword_from_fenced_code_block` (`src/adapters/kind.rs`); `flatten_includes_pyright_self_attribute_as_a_child`, `flatten_keeps_tsserver_overload_duplicates_as_separate_entries` (`src/indexer/symbol.rs`); `unhandled_method_error_surfaces_pyrights_exact_envelope` (`src/lsp/client.rs`).

Not yet pinned: the `typeDefinition`/`implementation` null-vs-`[]` semantics, the stdlib `.pyi`/`.py` double-`Location` split, and tsserver's `outgoingCalls.to` resolving to an interface method — these describe methods semnav doesn't call yet (`type_of`/`implements` edges are unimplemented), so there's no parser to pin them against; capturing them is deferred to whichever theme implements those edges.
