...
This is **not** a degradation signal in the sense of "the server is unhealthy" — the server is up and being used right now. It means a background refresh is running: either a *warm* anchor (materialized before) served from the cache, or a *cold* anchor just completed its first inline materialization and spawned a follow-up refresh — in both cases, a fresh materialization runs in the background, so the result may be missing a caller/reference. Re-querying picks up the converged answer once the background refresh completes. Cold anchors also spawn a refresh because the first inline materialization may be silently incomplete when the LSP server's background analysis hasn't finished yet (observed with pyright on large repos). `find_definition`/`find_symbol`/`find_callees` never set this field — `find_callees` uses a precise content-hash cache instead ([lsp-integration.md](./lsp-integration.md)), so its cached answer needs no freshness signal at all.
