...
mod tests {
    ...
    async fn index_language_real_custom_adapter_via_pyright() {
        // SAFETY: this test owns the full lifecycle of these vars (set at the
        // start, removed at the end) and no other test touches this
        // "fakelang" tag, so there's no cross-test race on the same key.
        unsafe {
